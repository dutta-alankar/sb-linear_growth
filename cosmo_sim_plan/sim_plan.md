...
The whole comparison — adiabatic evolution from either `InitGasTemp` choice
against the TH2010 temperature with its partial Compton coupling — is
generated by **`compare_temperature.py`** in this directory
(`python compare_temperature.py --z-start 200`), which prints the error
table above for any start redshift and produces
`temperature_comparison.png` (T_b(z) for the TH2010 fit, the fully-coupled
limit, and the two adiabatic tracks, plus the c_s ratio panel).
